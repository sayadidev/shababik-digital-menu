...
// Preloaded — categories are embedded from the server via a static default
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!itemRes.success || !itemRes.data) {
        setError(itemRes.error ?? "Failed to create item");
        setSaving(false);
        return;
      }
      ...
      if (variantInputs.length > 0) {
        ...
        if (!varRes.success) {
          setError(varRes.error ?? "Item created but variant save failed");
          setSaving(false);
          return;
        }
      }
